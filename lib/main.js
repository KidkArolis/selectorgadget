...
define(["jquery", "./dom"], function (jQuery, DomPredictionHelper) {
...
SelectorGadget.prototype.onChangeSelector = function () {};

SelectorGadget.prototype.setSelector = function (selector, options) {
  options = options || {};
  this.clearSelected();
  this.suggestPredicted(selector);
  this.setPath(selector, options);
};
...
SelectorGadget.prototype.sgMouseover = function(e) {
  ...
  if (self.parents(".sg_ignore").length) return;
...
SelectorGadget.prototype.sgMousedown = function(e) {
  ...
  if (w_elem.parents(".sg_ignore").length) return;
...
SelectorGadget.prototype.setupEventHandlers = function() {
  ...
  jQuery("body").on("mouseover.sg", "*:not(.sg_ignore)", { "self": this}, this.sgMouseover);
  jQuery("body").on("mouseout.sg", "*:not(.sg_ignore)", { "self": this}, this.sgMouseout);
  jQuery("body").on("mousedown.sg", "*:not(.sg_ignore)", { "self": this}, this.sgMousedown);
  jQuery("html").on("keydown.sg", { 'self': this }, this.listenForActionKeys);
  jQuery("html").on("keyup.sg", { 'self': this }, this.clearActionKeys);
};
...
SelectorGadget.prototype.removeEventHandlers = function() {
  ...
  jQuery("*").off(".sg");//, this.sgMouseover);
  jQuery("html").off(".sg");//, this.listenForActionKeys);
};
...
SelectorGadget.prototype.suggestPredicted = function(prediction) {
  if (prediction && prediction != '') {
    ...
    jQuery(prediction).each(function() {
      count += 1;
      var $el = jQuery(this);
      if (!$el.parents(".sg_ignore").length && !$el.hasClass('sg_selected') && !$el.hasClass('sg_ignore') && !$el.hasClass('sg_rejected')) {
        jQuery(this).addClass('sg_suggested');
      }
    });
...
SelectorGadget.prototype.setPath = function(prediction, options) {
  options = options || {};
  ...
  if (!options.silent) {
    this.onChangeSelector(prediction);
  }
};
...
SelectorGadget.prototype.makeInterface = function() {
  ...
  this.sg_div.append(jQuery('<input type="button" value="X"/>').bind("click", {'self': this}, this.destroy).addClass('sg_ignore'));
...
SelectorGadget.prototype.destroy = function(e) {
  var self = (e && e.data && e.data.self) || this;
  self.unbound = true;
...
// no auto initialisation anymore:
